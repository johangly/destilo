...
function validateRolPermissions(role, endpoint) {
  const ADMIN_ROLE = process.env.ADMIN_ROLE;
  const endpointsToCheck = ['/security-questions', '/customers', '/sells'];

  if (ADMIN_ROLE.toString() === role.toString()) {
    return true;
  } else {
    if ('employee' === role.toString()) {
      if (endpointsToCheck.some(path => endpoint === path)) {
        return true;
      } else {
        return false;
      }
...
export async function apiClient(role, endpoint, options = {}) {
  ...
  if(!isGetMethod){
    ...
    if (!validateRolPermissions(role, endpoint)) {
      return { message: 'error: Falta permisos para realizar esta accion' }
    }
  }
...
export async function recoveryPassword(endpoint, options = {}) {
  ...
  try {
    ...
    console.log(response,'respuesta del backend original')
    const data = await response.json();
    if (!response.ok) {
...
export const api = {
  ...
  // security questions
  createSecurityQuestions: (role, data) => apiClient(role, '/security-questions', {
    method: 'POST',
    body: JSON.stringify(data)
  }),
  validateSecurityQuestions: (data) => handleLogin('/security-questions/validate', {
    method: 'POST',
    body: JSON.stringify(data)
  }),
  getSecurityQuestionsByEmail: (email) => recoveryPassword(`/security-questions/getSecurityQuestionsByEmail/${email}`),
  
  getSecurityQuestions: (role, id) => apiClient(role, `/security-questions/${id}`),
